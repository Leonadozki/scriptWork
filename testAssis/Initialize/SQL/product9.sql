...
as
begin
	select item_no ItemNo,item_name ItemName,unit_no ItemUnit,item_size ItemSize,sale_price Price from bi_t_item_info where item_clsno=@clsno and 
	 (item_no like '%'+@keyword+'%' or item_name like '%'+@keyword+'%')
end
go
...
go
create proc[dbo].[kmmicro_pr_get_clslist]
as
Begin
...
go
create procedure kmmicro_Pr_set_WXPaymentFlow(@SheetNo varchar(30),@PayStatus char(1),@PayDate smalldatetime,@paytype varchar(5),@cVIP_C varchar(50))
as
begin
