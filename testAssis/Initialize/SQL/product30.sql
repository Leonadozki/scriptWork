...
go
create  procedure [dbo].[kmmicro_pr_insert_pos_sale] (@flow_no 		varchar(30), -- 微信单号
...
begin
   declare  @g_branch_no         varchar(15),
			...
			@vip_no              varchar(20)
	if not exists (select 1 from sys_t_system where sys_var_id = 'kmmicro_flowno' )
	begin
		insert into sys_t_system (sys_var_id, sys_var_name, sys_var_value, is_changed, sys_var_desc, km_sysflag)
		values ('kmmicro_flowno', '微商城流水号', '0', '是', '微商城流水号', '0')
	end
...
--	exec kmmicro_pr_get_pos_flowno @cash_no,@ls_tmp,@branch_no,@flow_no_pos output,@trade_date output
	declare @max_flow_no varchar(20),
            ...
            @branch_flow    varchar(4),
            @date_flow     varchar(8)
    select @date_flow=left(@ls_tmp,4)+substring(@ls_tmp,6,2)+substring(@ls_tmp,9,2)
	select @max_flow_no = right(sys_var_value, 4) from sys_t_system where sys_var_id = 'kmmicro_flowno'
    select @max_flow_no=@max_flow_no + 1
    if @max_flow_no is null or @max_flow_no=''  set @max_flow_no='1'
    select @max_flow_no=replicate('0',4 - len(@max_flow_no))+str(@max_flow_no,len(@max_flow_no))
    select @flow_no_pos=@cash_no+@date_flow+@max_flow_no,
           @trade_date=@date_flow
	...
	begin
	   ...
	   values (@li_i1,@flow_no_pos,@branch_no,@item_no,'', '', @sale_price,@sale_price,
              ...
              @batch_no,@cash_no,@vip_no,@pool_flag,0,'',@item_subno,'1', '微商城订单')
	   if @@error !=0
	   begin
	...
	begin
	  ...
	  insert into pos_t_saleflow ( flow_id, flow_no, branch_no, item_no, item_clsno, sup_no, source_price, sale_price, 
	   ...
	   values (@li_i1,@flow_no_pos,@branch_no,@item_no,'', '', @sale_addmoney,@sale_addmoney,
              ...
              @batch_no,@cash_no,@vip_no,@pool_flag,0,'',@item_subno,'1', '微商城订单外送费')
	end 	
	--end生成销售流水
	--生成付款流水
	select @flow_id=0
	...
	begin
	   ...
	   select @li_i2,@flow_no_pos,@sale_money,@branch_no,pay_way = (case pay_way when 'E' THEN 'V' WHEN 'V' THEN 'Z' else pay_way end),sell_way,card_no,@vip_no,
             coin_no,1.0000,pay_amount,@oper_date,@cashier_no,'9999','9999',
             @trade_date,@batch_no,@cash_no, '微商城订单'
	...
	deallocate cur1
   --生成付款流水end 
	update sys_t_system set sys_var_value = @max_flow_no where sys_var_id = 'kmmicro_flowno'
	--commit tran t
	return
...
go
 ...
 Begin 
	...
	SELECT @ic_flag = ic_cardflag FROM POS_T_VIP_INFO WHERE card_id = @ls_cardid
	IF UPPER(@ic_flag) = 'I' 
	BEGIN
...
-- execute wx_pr_insertvip '测试会员', '女', '13011011211', '1995-01-01', '', 'weixinhao'
 ...
 begin 
	...
	select @vipno = card_id from pos_t_vip_info where isnull(other3,'') = @openid and isnull(@openid,'') <> '' 
  	set @vipno = isnull(@vipno,'')   
	if @vipno <> ''  
  	BEGIN
...
Begin
	...
	Values ('0000', Getdate(), @card_id, -@trans_amt, '消费', '微信订单号:' + @order_no, -@trans_amt, @branch_no)
	If @@error <> 0
	Begin
...
begin  
	...
	set @trade_date = CONVERT(VARCHAR(4),DATEPART(YEAR, MAX(getdate()))) + RIGHT('00'+CONVERT(VARCHAR(2),DATEPART(MONTH, MAX(getdate()))),2) + RIGHT('00'+CONVERT(VARCHAR(2),DATEPART(DAY, MAX(getdate()))),2)
	INSERT INTO pos_t_vip_cost ( oper_id , oper_date , card_id , card_cost , card_way , reverse_desc ,actual_payamt ,branch_no ,  
				   card_fullfront , card_fullback, other1 ) 
    VALUES ('0000', Getdate(), @vip_no, @fillmoney + @presentmoney, '充值' , @desc, @fillmoney , @branch_no , 
		 @card_preamt ,  @card_balance , '微信充值')
	If @@error <> 0
	Begin
...
Begin
	...
	Values (@card_id, @mode, Getdate(), '0000', @type+'[' + Convert( Varchar, Convert( Int, @add_integral ) ) + ']--会员[' + @card_id + ']', 
		@sum_saleamt, @sum_integral, 0, @add_integral, '线上平台加减积分', '', @branch_no)
	If @@error <> 0
	Begin
...
go
 ...
 --根据会员卡号，原密码， 修改密码
--参数：会员卡号、原密码、新密码
--exec kmmicro_Pr_alter_vip_pw '0100002', '翁淌翁', '翁淌嗜'
...
begin 
	...
	insert into sa_t_alloper_log ( oper_id, log_flag, oper_date,trade_type,module_name,description,memo,computer_name, machine_no, item_no, sup_no, vip_no, sheet_no, other1, other2, other3)
	values ('0000','H',getdate(),'更改卡密码','会员更改卡密','微信助手修改会员密码:原密码[' + @oldpw + '],新密码[' + @newpw + ']','线上修改密码',@computer, '', '', '', @cardid, '', '', '', '')
...
/*
chenhd，2015-12-24
过程名： kmmicro_pr_get_iteminfo
参数：itemno varchar(50)
返回：select bi.item_no ItemNo,bi.item_name ItemName,isnull(bi.unit_no,'') ItemUnit,bi.sale_price Price ,isnull(bi.item_size,'') ItemSize, 
...
if exists( select * from sysobjects where xtype = 'P' and name = 'kmmicro_pr_get_iteminfo')
	DROP PROCEDURE kmmicro_pr_get_iteminfo
go

-- execute kmmicro_pr_get_iteminfo '010129'
 create procedure kmmicro_pr_get_iteminfo(@itemno varchar(50)) 
 as 
 begin 
 	select item_no ItemNo,item_name ItemName,unit_no ItemUnit,item_size ItemSize,sale_price Price ,bi_t_item_cls.item_clsname Brand,product_area  Origin ,' ' ProductionDate,valid_days ShelfLife ,unit_no Packaging,item_size Size
	from bi_t_item_info left join bi_t_item_cls on (bi_t_item_info.item_brand = bi_t_item_cls.item_clsno and bi_t_item_cls.item_flag = '1')
